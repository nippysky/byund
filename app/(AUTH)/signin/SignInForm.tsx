...
export function SignInForm() {
  ...
  async function onSubmit(values: SignInValues) {
    ...
    const res = await fetch("/api/auth/login", {
      ...
      // same-origin cookies included by default for same-site requests,
      // but explicit is fine if you later move domains.
    ...
    // Cookie is set httpOnly on the response. Now we navigate.
  ...
  return (
    <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <p className="mt-4 text-xs text-muted">
        New to BYUND?{" "}
        <Link href="/register" className="text-action">
          Create an account
        </Link>
        .
      </p>
